...
const authorizeUrl = `https://accounts.spotify.com/authorize?client_id=${client_id}&response_type=token&redirect_uri=${encodeURIComponent(redirect_uri)}&scope=${encodeURIComponent(scopes)}`;
...
const getUserID = async (accessToken) => {
  ...
  if (response.ok) {
    ...
    console.error('Failed to fetch user ID:', response.status, response.statusText, errorData);
  }
};
...
const getPlaylistISRC = async (accessToken, playlistId) => {
  ...
  if (response.ok) {
    ...
    data.items.forEach(item => {
      if (item.track && item.track.external_ids && item.track.external_ids.isrc) {
...
const getRecommendations = async (accessToken, tracksIds) => {
  const response = await fetch(`https://api.spotify.com/v1/recommendations?limit=${numRecommendations}&seed_tracks=${tracksIds.join(',')}`, {
    headers: {  
      'Authorization': `Bearer ${accessToken}`
    }
  });
  if (response.ok) {
    ...
    console.log(data.tracks.map(track  =>  
